...
#' excluded
#' @param variantIncludeFile a one column file specifying the variants to be 
...
#' @param annotationUsed a one column file without the header listing 
...
#' v2genome, v3genome, v4exome, v4genome 
#'  if gnomAD is used
...
#' @param batchSize the number of genes/groups to process in each batch when regionFile is not set. This is used to control the memory usage
...
CoCoRV = function(controlGDSFile,
  ...
  variantInclude = NULL
  if (nchar(variantIncludeFile) > 0) {
    variantInclude = fread(variantIncludeFile,data.table = F, header = F, 
                           colClasses="character")[, 1]
  }
  ...
  if (annotationUsed == "") {
    if (variantGroup %in% definedSet) {
      ...
      cat("!! All annotations from data will be loaded. It is much faster and more memory efficient to specify needed annotations by using the option annotationUsed\n")
      annotationSet = NULL
    }
  } else {
    annotationSet = c(as.matrix(
             read.table(annotationUsed, header = F, as.is = T)))
    if (groupColumn != "NA") {
        annotationSet = union(annotationSet, groupColumn)
    }
  }
  ...
  if (!is.null(regionGRange)) {
    ...
    # iterate through the IDs defined by groupColumn

    # generate the regions for individual IDs of the groupColumn
    if (!is.null(caseAnnoGDS)) {
      gds = caseAnnoGDS
    } else {
      gds = caseGDS
    }
    caseGroupData = seqGetData(gds, c("chromosome", "position"))
    groupColumnData = seqGetData(gds, 
                            paste0("annotation/info/", groupColumn))
    if (class(groupColumnData)[1] == "SeqVarDataList") {
        # deal with SeqVarDataList from multiple genes separated by comma
        groupColumnData = seqListVarData(groupColumnData)
        groupColumnData = sapply(groupColumnData, function(x) {
                                if (length(x) > 1)
    ...
    caseGroupData[[groupColumn]] = groupColumnData
    ...
    controlGroupData = seqGetData(gds, c("chromosome", "position"))
    groupColumnData = seqGetData(gds, 
                            paste0("annotation/info/", groupColumn))
    if (class(groupColumnData)[1] == "SeqVarDataList") {
        # deal with SeqVarDataList from multiple genes separated by comma
    ...
    controlGroupData[[groupColumn]] = groupColumnData

    caseGroupData = data.frame(caseGroupData)
    controlGroupData = data.frame(controlGroupData)
    groupData = rbind(caseGroupData, controlGroupData)
    colnames(groupData)[3] = "groupID"

    # aggregate to find the start and end 
    setDT(groupData)
    minmax = groupData[, 
                .(min_value = min(position), max_value = max(position)), 
                by = .(groupID, chromosome)]
    aggregatedGroups = as.data.frame(minmax)

    # extend 150 BP for both start and end to allow for long ALT alleles
    aggregatedGroups[, 3] = pmax(1, aggregatedGroups[, 3] - 150)
    aggregatedGroups[, 4] = aggregatedGroups[, 4] + 150

    aggregatedGroups = aggregatedGroups[
                  order(aggregatedGroups[, 2], aggregatedGroups[, 3]), , 
                  drop = F]
    groupID = unique(aggregatedGroups[, 1])

    batchIndex = seq(from = 1, by = batchSize, to = length(groupID) + 1)
    if (batchIndex[length(batchIndex)] != length(groupID) + 1) {
      batchIndex = c(batchIndex, length(groupID) + 1)
    }
    for (i in 1:(length(batchIndex) - 1)) {
      groupIDInBatch = groupID[batchIndex[i]:(batchIndex[i + 1] - 1)]

      index = match(groupIDInBatch, aggregatedGroups[, 1])
      regionGRange = GRanges(seqnames = aggregatedGroups[index, 2],
                         ranges = IRanges(
                              start = aggregatedGroups[index, 3],
                              end = aggregatedGroups[index, 4]),
                          strand = Rle(strand("+"), length(index)))
      bedPerRegion = intersect(regionGRange, bedGRange)
      if (length(bedPerRegion) > 0) {
        result = CoCoRVPerRegion(controlGDS,
                             caseGDS,
                             controlAnnoGDS = controlAnnoGDS,
...
CoCoRVPerRegion = function(controlGDS,
  ...
  if (!is.null(controlGDS)) {
    ...
    cat("loading control count data ...\n")
  ...
  caseAnno = NULL
  if (!is.null(caseAnnoGDS)) {
    cat("loading case annotation data ...\n")
  ...
  controlAnno = NULL
  if (!is.null(controlAnnoGDS)) {
    cat("loading control annotation data ...\n")
  ...
  if (length(controlAC) == 1) {  # one group
    ...
    if (AFGroup == "pool") {
      ...
      controlAFMatrix[is.nan(controlAFMatrix)] = 0
      caseAFMatrix[is.nan(caseAFMatrix)] = 0
  ...
  # check whether some PAR variants spill over to nonPAR regions, so 
  # the max is not fair for nonPAR regions and they may be filtered out
  if (sum(variantInNonPAR) > 0) {
    controlANMax[variantInNonPAR] = quantile(controlANs[variantInNonPAR], 0.95)
  }
  ...
  if (sum(variantInNonPAR) > 0) {
    caseANMax[variantInNonPAR] = quantile(caseANs[variantInNonPAR], 0.95)
  }
  ...
  indexControlRemove = (controlAF > AFMax) & 
                 !(controlCountQC[, 1] %in% variantInclude)
  ...
  indexCaseRemove = indexCaseRemove & !(caseCountQC[, 1] %in% variantInclude)
  ...
  variantExclude = setdiff(variantExclude, variantInclude)
  ...
  if (dim(caseCountQC)[1] == 0) {
    print("no variants left after filtering in cases, please check")
  }

  if (dim(controlCountQC)[1] == 0) {
    print("no variants left after filtering in controls, please check")
  }  

  if (dim(caseCountQC)[1] == 0 && dim(controlCountQC)[1] == 0) {
    return(NULL)
  }
  ...
  caseFunction = variantSets(caseCountQC, extraParamJason, 
                             variantGroupCustom,
                             variantGroup, minREVEL, maxAFPopmax) | 
                    (caseCountQC[, 1] %in% variantInclude)
  if (sum(controlFunction) == 0 && sum(caseFunction) == 0) {
    print("no variants selected in either case or control")
    return(NULL)
  }
  ...
  ## process each gene
  if (groupColumn == "NA") {  
    print("groupColumn is NA, all variants will be considered")
    groupColumn = "__tempGroup__"
    controlCountQC[, "__tempGroup__"] = "tempGroup"
    caseCountQC[, "__tempGroup__"] = "tempGroup"
  }
  geneList = union(unique(controlCountQC[, groupColumn]), 
                   unique(caseCountQC[, groupColumn]))

  # restrict to the group specified
  if (!is.null(groupSpecified)) {
    geneList = intersect(geneList, groupSpecified)
  }

  # remove blank names
  only_blank = grepl("^\\s*$", geneList)
  geneList = geneList[!only_blank]
  ...
  if (nCase == -1 || nControl == -1) {
    ...
    if (sum(!caseInNonPAR) > 0 & sum(!controlInNonPAR) > 0) {
      # use variants outside XYNonPAR to estimate the sample count 
      for (i in 1:length(controlAN)) {
        ...
        nControl = ceiling(
          max(controlCountQC[!controlInNonPAR, controlAN[i]]) / 2)
        print(paste("group:", ACANConfigData[i, "caseGroup"],
                    "cases:", nCase, "controls:", nControl))
      }
    } else if (sum(!caseInNonPAR) == 0 & sum(!controlInNonPAR) == 0) {
      # all variants are XYNonPAR variants
      for (i in 1:length(controlAN)) {
        if (extractStratifiedCounts) {
          nCase = ceiling(max(caseACANExtracted[[2]][, i]) / 2)
        } else {
          nCase = ceiling(max(caseCountQC[, caseAN[i]]) / 2)
        }
        nControl = ceiling(
          max(controlCountQC[, controlAN[i]]) / 2)
        # assume *XX group means the female group, *XY group the male group
        # as used by gnomAD 
        if (regexpr("XY", controlAN[i])) {
          nCase = nCase * 2
          nControl = nControl * 2
        }
        print(paste("group:", ACANConfigData[i, "caseGroup"],
      ...
      stop("Either cases or controls have all XYNonPAR variants but not both")
    } 
  ...
  # adjust the male group if all variants are from XYNonPAR
  if (sum(!caseInNonPAR) == 0 & sum(!controlInNonPAR) == 0) {
    for (i in 1:nGroup) {
      if (regexpr("XY", controlACANConfig[i, 2])) {
        nControlPerGroup[, i] = nControlPerGroup[, i] * 2
        if (is.null(caseGroupInfo)) {
          nCasePerGroup[, i] = nCasePerGroup[, i] * 2
        }
      }
    }
  }
  ...
  if (!ignoreEthnicityInLD) {
    if (gnomADVersion %in% c("v2exome", "v3genome", "v4exome", "v4genome")) {
      ...
      stop("gnomAD version should be: v2exome, v2genome, v3genome, v4exome, v4genome")
    }
  ...
  if (regexpr("sample", countUse) != -1) {
    ...
    colnames(dominantResult) = paste0(colnames(dominantResult), "_DOM")
    recessiveResult = contingencyTableTest(recessiveTable, test)
    colnames(recessiveResult) = paste0(colnames(recessiveResult), "_REC")
    twoHetsResult = contingencyTableTest(twoHetsTable, test)
    colnames(twoHetsResult) = paste0(colnames(twoHetsResult), "_2HET")
