...
main = function() {
  ...
  parser$add_argument("controlGDSFile", 
                      help = "gds format file of controls")
  parser$add_argument("caseGDSFile", 
                      help = "gds format file of cases")
  parser$add_argument("--controlAnnoGDSFile", metavar='',
                      action = "store", default = "",
                      help = "gds format file of annotated variants in controls")
  parser$add_argument("--caseAnnoGDSFile", metavar='',
                      action = "store", default = "",
                      help = "gds format file of annotated variants in cases")
  ...
  parser$add_argument("--variantIncludeFile", metavar='',
                      action = "store", default = "",
                      help = paste("a one column file specifying the variants", 
                        "to be included, it will also include variants", 
                        "specified in variantExcludeFile"))
  ...
  parser$add_argument("--variantGroup", metavar='',
                      ...
                      help = paste("a specified variant group to use for test or a self defined",
                                   ...
                                   "more details"))
  parser$add_argument("--annotationUsed", metavar='',
                      action = "store", default = "",
  ...
  parser$add_argument("--gnomADVersion", metavar='',
                      ...
                      help = paste("if gnomAD is used, specify the version:", 
                        "v2exome, v2genome, v3genome, v4exome, v4genome"))
  ...
  parser$add_argument("--batchSize", metavar='',
                      action = "store", type = "integer", default = 10000,
                      help = "the number of genes/groups to process in each batch when regionFile is not set. This is used to control the memory usage")
  ...
  parser$add_argument("--fileID", metavar='', 
                      action = "store", default = "",
                      help = paste("the file ID to link back to the input case and control files, used to extract variant information later for top genes. If the input files are split by chromosome, then use chromosome ID as the file ID."))
  ...
  if (length(resultPerRegion) > 0) {
    ...
    for (i in 1:length(resultPerRegion)) {
      ...
      if (fileID != "") {
        # add the file ID
        result$testResult = cbind(fileID, result$testResult)
      }
